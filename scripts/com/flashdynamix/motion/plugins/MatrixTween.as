   ...
   public class MatrixTween extends AbstractTween
   {
      ...
      override public function update(param1:Number) : void
      {
         ...
         for(_loc3_ in propNames)
         {
            if(_loc3_ == "tx")
            {
               this._current.tx = this._from.tx * _loc2_ + this._to.tx * param1;
            }
            else if(_loc3_ == "ty")
            {
               this._current.ty = this._from.ty * _loc2_ + this._to.ty * param1;
            }
            else if(_loc3_ == "a")
            {
               this._current.a = this._from.a * _loc2_ + this._to.a * param1;
            }
            else if(_loc3_ == "b")
            {
               this._current.b = this._from.b * _loc2_ + this._to.b * param1;
            }
            else if(_loc3_ == "c")
            {
               this._current.c = this._from.c * _loc2_ + this._to.c * param1;
            }
            else if(_loc3_ == "d")
            {
               this._current.d = this._from.d * _loc2_ + this._to.d * param1;
            }
            else
            {
